...
router.post(
  ...
  [
    ...
    body("gender", "Please select a valid gender").isIn([
      "male",
      "female",
      "other",
    ]),
    body("city", "Please enter a valid city").isLength({ min: 3 }),
    body("state", "Please enter a valid state").isLength({ min: 3 }),
    body(
      "howDidYouHear",
      "Please provide information on how you heard about us"
    ).isLength({ min: 5 }),
  ],
  async (req, res) => {
    let success = false;
